...
class ArchiveHabitUseCaseTest {

    private lateinit var habitRepository: HabitRepository
    private lateinit var useCase: ArchiveHabitUseCase
    ...
    fun setup() {
        ...
        useCase = ArchiveHabitUseCase(habitRepository)
    }

    @Test
    fun `archive active habit succeeds and sets archivedAt`() = runTest {
        ...
        val archived = (result as Result.Success).value
        assertTrue(archived.status is HabitStatus.Archived)
        assertNotNull(archived.archivedAt)
    }

    @Test
    fun `archive paused habit succeeds`() = runTest {
    ...
    @Test
    fun `archive from Archived status is rejected`() = runTest {
        val habit = activeHabit().copy(
        ...
        assertTrue((result as Result.Error).message.contains("already archived"))
        coVerify(exactly = 0) { habitRepository.update(any()) }
    }

    @Test
    fun `archive non-existent habit returns error`() = runTest {
        val habit = activeHabit()
