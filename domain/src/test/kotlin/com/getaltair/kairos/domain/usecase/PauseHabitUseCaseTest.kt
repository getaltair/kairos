...
class PauseHabitUseCaseTest {

    private lateinit var habitRepository: HabitRepository
    private lateinit var useCase: PauseHabitUseCase
    ...
    fun setup() {
        ...
        useCase = PauseHabitUseCase(habitRepository)
    }

    @Test
    fun `pause active habit succeeds and sets pausedAt`() = runTest {
        ...
        val paused = (result as Result.Success).value
        assertTrue(paused.status is HabitStatus.Paused)
        assertNotNull(paused.pausedAt)
    }

    @Test
    fun `pause from Paused status is rejected`() = runTest {
    ...
    @Test
    fun `pause from Archived status is rejected`() = runTest {
        val habit = activeHabit().copy(
    ...
    @Test
    fun `pause non-existent habit returns error`() = runTest {
        val habit = activeHabit()
