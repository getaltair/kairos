...
6. **Check for spec template docs** (see `core/skills/plan-from-specs/SKILL.md`):
   - Search `docs/specs/` for `04-architecture.md`, `05-erd.md`, `06-state-machines.md`
   - Read for architecture decisions (look for `<!-- ADR-CANDIDATE -->` markers)
   - Read ERD for database schemas and contract surfaces
   - Read state machines for entity lifecycle definitions
   - Also check `08-SUB-*` for relevant subsystem designs
