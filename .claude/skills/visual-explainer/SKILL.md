...
**Output location:** Pick the directory based on what is being visualized:

- **Feature-scoped visuals** (the input file lives inside `Context/Features/NNN-Name/`, e.g. visualizing `Context/Features/007-Auth/Steps.md`): write to `Context/Features/NNN-Name/Views/` so the visualization travels with the feature. Create the directory with `mkdir -p` if needed.
- **Project-level visuals** (architecture overviews, project recaps, ad-hoc diagrams not tied to a single feature): write to `.cortex/views/` (gitignored, ephemeral, persists across sessions).

Use a descriptive filename based on content: `plan-review-auth-system.html`, `pipeline-flow.html`, `schema-overview.html`.

**Open in browser (cross-platform):**

- Windows: `start .cortex/views/filename.html` (in Git Bash) or `start "" ".cortex\views\filename.html"` (in cmd)
- macOS: `open .cortex/views/filename.html`
- Linux: `xdg-open .cortex/views/filename.html`
