...
3. **Check for spec template docs** (see `core/skills/plan-from-specs/SKILL.md`):
   - Search `docs/specs/`, `specs/`, or project root for `01-PRD-*` files
   - If found, read the relevant PRD for pre-written requirements and assertions
   - Also check for `03-invariants.md` to pull relevant business rules
   - If spec docs exist, use them as primary source; generate only what's missing
4. If the feature area is unclear, ask the user for clarification before proceeding
